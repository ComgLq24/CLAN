...
class GTA5DataSet(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            row, column, _ = label.shape
            label = label.reshape(-1, 3)
            label = np.asarray(list(map(lambda x: '#%02x%02x%02x' % tuple(x), label.tolist()))).reshape(row, column)
            label_copy = 255 * np.ones((row, column), dtype=np.int32)
